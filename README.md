...
3. Create an SSH key in `SSH Keys`.
...
The private key never leaves the local gateway.
...
- SSH private keys are generated and stored by the local gateway.
...
- `AIPERMISSION_GATEWAY_SECRET` can be omitted for automatic generation. If it is set explicitly, use at least 32 random characters. The local-development placeholder is replaced by a generated high-entropy secret at startup.
