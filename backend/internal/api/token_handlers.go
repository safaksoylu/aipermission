...
func (s tokenHandlers) listTokenPermissions(w http.ResponseWriter, r *http.Request) {
	runtime, ok := s.activeRuntimeOrLocked(w)
	if !ok {
		return
	}
	...
	items, err := runtime.tokens.ListPermissions(r.Context(), id)
	if err != nil {
		handleTokenError(w, err)
		return
	}
...
func (s tokenHandlers) updateTokenPermissions(w http.ResponseWriter, r *http.Request) {
	runtime, ok := s.activeRuntimeOrLocked(w)
	if !ok {
		return
	}
	...
	var request tokens.UpdatePermissionsRequest
	if err := decodeJSON(w, r, &request); err != nil {
		writeError(w, http.StatusBadRequest, "invalid json body")
		return
	}

	items, err := runtime.tokens.UpdatePermissions(r.Context(), id, request)
	if err != nil {
		handleTokenError(w, err)
		return
	}
	s.writeAudit(r.Context(), runtime, "user", nil, 0, "token.permissions.updated", map[string]any{
		"token_id":    id,
