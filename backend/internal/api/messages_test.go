...
func TestMessagesValidateListConsumeAndMarkRead(t *testing.T) {
	...
	first, err := fixture.server.insertMessage(ctx, runtime, createMessageRequest{TokenID: token.ID, RuntimeID: &server.ID, Message: " first token=secret-value "})
	if err != nil {
	...
	missingRuntimeID := otherServer.ID + 999
	if _, err := fixture.server.insertMessage(ctx, runtime, createMessageRequest{TokenID: token.ID, RuntimeID: &missingRuntimeID, Message: "missing server"}); err == nil || !strings.Contains(err.Error(), "runtime_id") {
		t.Fatalf("expected unknown server to fail, got %v", err)
	}
	if _, err := fixture.server.insertMessage(ctx, runtime, createMessageRequest{TokenID: token.ID, RuntimeID: &otherServer.ID, Message: "other server"}); err != nil {
		t.Fatalf("insert other server message: %v", err)
	}
	second, err := fixture.server.insertMessage(ctx, runtime, createMessageRequest{TokenID: token.ID, RuntimeID: &server.ID, Direction: "ai_to_user", Message: "second"})
	if err != nil {
	...
	response := performJSON(fixture.server.Handler(), "POST", "/api/messages/read", "", markMessagesReadRequest{RuntimeID: server.ID})
...
func TestMessagesPreferMatchingSessionScope(t *testing.T) {
	...
	otherServer := fixture.createKeyAndServer(t, "worker-2")
	if _, err := fixture.server.insertMessage(ctx, runtime, createMessageRequest{TokenID: token.ID, RuntimeID: &otherServer.ID, SessionID: &sessionOne, Message: "wrong server"}); err == nil || !strings.Contains(err.Error(), "session_id") {
		t.Fatalf("expected mismatched session/server to fail, got %v", err)
	}
	if _, err := fixture.server.insertMessage(ctx, runtime, createMessageRequest{TokenID: token.ID, RuntimeID: &server.ID, SessionID: &sessionOne, Message: "session one"}); err != nil {
		t.Fatalf("insert session one message: %v", err)
	}
	if _, err := fixture.server.insertMessage(ctx, runtime, createMessageRequest{TokenID: token.ID, RuntimeID: &server.ID, Message: "server scoped"}); err != nil {
	...
	if _, err := fixture.server.insertMessage(ctx, runtime, createMessageRequest{TokenID: token.ID, RuntimeID: &server.ID, SessionID: &sessionTwo, Message: "session two"}); err != nil {
